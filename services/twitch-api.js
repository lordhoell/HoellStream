...
// --- Normalizer: Twitch -> overlay-event ---
function normalizeTwitchEvent(e) {
  if (!e || !e.type) return null;
  const t = (e.type || '').toLowerCase();

  // standardize bits field to a number for "? bits" issue
  const bitsNum = e.bits != null ? Number(e.bits) : (e.amount != null ? Number(e.amount) : 0);
  ...
  switch (t) {
    case 'follow':
      return {
        platform: 'twitch',
        type: 'follow',
        displayName: e.displayName || e.username || 'Unknown',
        username:    e.username || e.displayName || 'Unknown',
        avatarUrl:   e.avatar || e.avatarUrl || (e.author && e.author.avatar) || '',
        ts: Date.now()
      };

    case 'subscription': // non-gift sub
      if (e.isGift) return null; // gift subs normalized below
      return {
        platform: 'twitch',
        type: 'sub',
        displayName: e.displayName || e.username || 'Unknown',
    ...
    case 'gift_purchase':
    case 'submysterygift':
      // Mass gift purchase event (gifter announcing they gifted X subs)
      return {
        platform: 'twitch',
        type: 'gift_sub',
        from: e.displayName || e.username || 'Unknown',
        fromDisplayName: e.displayName || e.username || 'Unknown',
        fromAvatarUrl: e.avatar || e.avatarUrl || '',
        to: null, // No specific recipient for mass gifts
        count: e.giftCount || e.count || 1,
        tier: e.tier || null,
        ts: Date.now()
      };
    ...
    case 'subgift':
    case 'gift_sub':
      // Individual gift recipient event
      return {
        platform: 'twitch',
        type: 'gift_sub',
        from: e.gifterDisplayName || e.gifter || 'Unknown',
        fromDisplayName: e.gifterDisplayName || e.gifter || 'Unknown',
        fromAvatarUrl: e.gifterAvatar || '',
        to: e.username || e.displayName || null,
        toDisplayName: e.displayName || e.username || null,
        toAvatarUrl: e.avatar || e.avatarUrl || '',
        count: 1, // Individual gifts are always count of 1
        tier: e.tier || null,
        ts: Date.now()
      };
    ...
    case 'cheer':
      return {
        ...
        displayName: e.displayName || e.username || 'Unknown',
        username:    e.username || e.displayName || 'Unknown',
        amount: bitsNum,             // numeric; fixes "? bits"
        avatarUrl: e.avatar || e.avatarUrl || (e.author && e.author.avatar) || '',
        message: '',                 // omit message (you requested)
        ts: Date.now()
      };
    ...
    case 'raid':
      return {
        platform: 'twitch',
        type: 'raid',
        raider:  e.raider || e.displayName || e.username || 'Unknown',
        viewers: e.viewers || e.viewerCount || 0,
        ts: Date.now()
      };
...
class TwitchAPIService {
  ...
  async handleUserNotice(message) {
    ...
    switch (msgId) {
      ...
      case 'resub':
        ...
        this.events.push(subscriberEvent);
  ...
  broadcastUpdates() {
    ...
    // Also broadcast normalized overlay events via SSE
    if (data && Array.isArray(data.events)) {
      const normalized = data.events.map(normalizeTwitchEvent).filter(Boolean);
      normalized.forEach(evt => broadcastOverlay(evt));
    }
  }
