...
// Helper to clean names (strip leading @)
function cleanName(name) {
  if (!name) return 'Unknown';
  return String(name).replace(/^@+/, '');
}

// --- Normalizer: YouTube -> overlay-event ---
function normalizeYouTubeEvent(e) {
  if (!e || !e.type) return null;
  const now = Date.now();
  const t = (e.type || '').toLowerCase();

  // Helpers from raw event
  const rawName  = (e.displayName || (e.author && e.author.name) || e.username || 'Unknown');
  const rawAvatar = (e.avatar || e.avatarUrl || (e.author && e.author.avatar) || '');

  // ---- Gifted membership (BATCH) ----
  const isGiftBatch =
    t.includes('gift') && (t.includes('member') || t.includes('membership')) &&
    (t.includes('purchase') || t.includes('gifted') || t === 'gift_member' || t === 'membershipgift');

  if (isGiftBatch) {
    const from = cleanName(e.from || e.gifter || rawName);
    return {
      platform: 'youtube',
      type: 'gift_member',
      from,
      displayName: from,             // <- used by overlay-ws left label
      avatarUrl: rawAvatar || '',    // <- populate avatar if we have it
      count: e.count || e.quantity || e.giftCount || e.membershipGiftCount || e.giftedMembershipsCount || 1,
      level: e.level || e.membershipLevel || null,
      ts: e.timestamp ? Date.parse(e.timestamp) || now : now
    };
  }

  // ---- Gifted membership (RECIPIENT) ----
  const isGiftReceived =
    (t.includes('received') && t.includes('gift') && (t.includes('member') || t.includes('membership'))) ||
    t === 'gift_membership_received' || t === 'giftedmembershipreceived';

  if (isGiftReceived) {
    const to   = cleanName(e.to || e.recipient || rawName);
    const from = cleanName(e.from || e.gifter || e.sourceName || (e.source && e.source.name) || 'Unknown');
    return {
      platform: 'youtube',
      type: 'gift_member_received',
      to,
      from,
      displayName: to,               // <- used by overlay-ws left label
      avatarUrl: rawAvatar || '',
      level: e.level || e.membershipLevel || null,
      ts: e.timestamp ? Date.parse(e.timestamp) || now : now
    };
  ...
  switch (t) {
    ...
    case 'superchatevent': {
      const name = cleanName(rawName);
      // IMPORTANT: omit message body for overlay-ws (amount only)
      return {
        platform: 'youtube',
        type: 'superchat',
        username: name,
        displayName: name,
        avatarUrl: rawAvatar || '',
        amount: e.amount || e.value || e.amountDisplayString || e.amountMicros || 0,
        currency: e.currency || null,
        message: '',  // <— blank on purpose (you asked to omit)
        ts: e.timestamp ? Date.parse(e.timestamp) || now : now
      };
    }
    ...
    case 'superstickerevent': {
      ...
      return {
        ...
        displayName: name,
        avatarUrl: rawAvatar || '',
        stickerUrl: e.stickerUrl || null,
        amount: e.amount || e.amountDisplayString || 0,
        currency: e.currency || null,
        ts: e.timestamp ? Date.parse(e.timestamp) || now : now
      };
    }
    ...
    default:
      return null; // ignore plain chat here
  }
}
...
class YouTubeAPIService {
  constructor() {
    ...
    // Set up callback for when new badges are found
    this.emojiScraper.setBadgeCacheUpdateCallback((newBadgeCount) => {
      this.handleBadgeCacheUpdate(newBadgeCount);
    });
  ...
  async start() {
    ...
    // Do an immediate backfill so emojis/badges are available right away (non-blocking)
    if (typeof this.emojiScraper.performBackfillScraping === 'function') {
      this.emojiScraper.performBackfillScraping();  // Run in background, don't await
    }
  ...
  async pollYouTubeData() {
    ...
    try {
      ...
      if (!this.liveChatId) {
        ...
        // Check if the stream ID is invalid
        if (liveChatResult && liveChatResult.invalid) {
          console.log('[YouTube API Service] ❌ Invalid stream ID detected - disconnecting and stopping service');
          this.connectionStatus = false;
          this.broadcastConnectionStatus();
    ...
    for (const message of messages) {
      ...
      switch (messageType) {
        case 'textMessageEvent':
          ...
          // Detect StreamElements-style "just subscribed!" notifications and convert to an event
          const authorName = (authorDetails.displayName || '').toLowerCase();
          if ((authorName === 'streamelements' || authorName === 'stream elements') && messageText && messageText.endsWith(' just subscribed!')) {
            const subscriberName = messageText.slice(0, -' just subscribed!'.length).trim();
            if (subscriberName) {
              newEvents.push({
                ...
                avatar: '',            // StreamElements doesn't give us the user's avatar
          ...
          const displayNameRaw =
            ...
            authorDetails.channelTitle ||
            authorDetails.authorName ||
            authorDetails.name ||
            '';
          const displayName = cleanName(displayNameRaw);
          
          let handle = null;
          // Try to capture the @handle without forcing it as the display name
          if (authorDetails.channelUrl && typeof authorDetails.channelUrl === 'string') {
            const last = authorDetails.channelUrl.split('/').pop();
            if (last && last.startsWith('@')) handle = last; // e.g. @johnsmith
          }

          const textMessage = {
            id: message.id,
            timestamp: snippet.publishedAt,
            displayName,                    // <= sanitized human name
            username: displayName,           // <= mirror for consistency
            handle: authorDetails.channelHandle || handle || null,  // optional, keep if you already have it
  ...
  // Helper to broadcast overlay events to both windows and SSE
  broadcastOverlay(evt) {
    try {
      const { BrowserWindow } = require('electron');
      BrowserWindow.getAllWindows().forEach((win) => {
        try { win.webContents.send('overlay-event', evt); } catch {}
      });
    } catch {}
  }
  ...
  broadcastUpdates() {
    ...
    // Normalize and broadcast overlay events ONCE (outside the window loop)
    if (data && Array.isArray(data.events)) {
      for (const raw of data.events) {
        const evt = normalizeYouTubeEvent(raw);
        if (evt) this.broadcastOverlay(evt);  // <-- now reaches overlay-ws via SSE too
      }
    }
  }
  ...
  // Broadcast badge cache updates to all windows
  handleBadgeCacheUpdate(newBadgeCount) {
    console.log(`[YouTube API Service] 🛡️ Badge cache updated with ${newBadgeCount} new badges!`);
    const { BrowserWindow } = require('electron');
    const allWindows = BrowserWindow.getAllWindows();
    allWindows.forEach((win) => {
      if (win && win.webContents && !win.isDestroyed()) {
        try {
          win.webContents.send('youtube-badge-cache-update', newBadgeCount);
        } catch (error) {
          console.error(`[YouTube API Service] Failed to send badge cache update to window ${win.id}:`, error);
        }
      }
    });
  }
